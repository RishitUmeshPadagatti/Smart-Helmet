...
/**
 * Test script to analyze video from uploads folder
 * Processes video using dual models and voting-based OCR
 */
    ...
    if (result.success) {
      ...
      if (analytics.plate_extraction) {
        ...
        if (plateInfo.success) {
          ...
          console.log(`✓ Valid Extractions from Frames: ${plateInfo.validExtractions}/${plateInfo.attempts}`);
      ...
      if (fs.existsSync(outputDir)) {
        ...
        files.forEach(file => {
          ...
          // List violation frames if directory exists
          if (file === 'violation_frames' && stat.isDirectory()) {
            const violationFiles = fs.readdirSync(filePath);
            violationFiles.forEach(vFile => {
              console.log(`    ├─ ${vFile}`);
            });
          }
        });
      ...
      return {
        success: true,
        licensePlate: analytics.plate_extraction?.plate || 'Not extracted',
        confidence: analytics.plate_extraction?.confidence || 0,
        annotatedVideo: path.join(outputDir, analytics.annotated_video || 'annotated_violations.mp4'),
...
analyzeVideo().then(summary => {
  if (summary.success) {
    ...
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log(`License Plate: ${summary.licensePlate}`);
    console.log(`Confidence: ${(summary.confidence * 100).toFixed(1)}%`);
    console.log(`Annotated Video: ${summary.annotatedVideo}`);
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
  }
